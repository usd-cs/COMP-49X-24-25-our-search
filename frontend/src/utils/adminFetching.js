...
import { ADMIN_EMAIL_URL, BACKEND_URL, GET_DEPARTMENTS_ENDPOINT, GET_DISCIPLINES_ENDPOINT, GET_RESEARCH_PERIODS_ENDPOINT, GET_UMBRELLA_TOPICS_ENDPOINT } from '../resources/constants'
...
export const handleAddEmail = async (newEmail, setNewEmail, setAdminEmails, setLoading, setError, getDataFrom) => {
  if (!newEmail.trim()) {
    setError('Error adding admin email. Cannot be blank.')
    return
  }
  ...
  try {
    const response = await fetch(ADMIN_EMAIL_URL, {
      credentials: 'include',
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
    ...
    const newData = await getDataFrom(ADMIN_EMAIL_URL)
    if (newData.length === 0) {
      throw new Error('505')
    } else {
      setAdminEmails(newData)
    }
...
export const handleDeleteEmail = async (id, setLoading, AdminEmails, setAdminEmails, setDeletingId, setOpenDeleteDialog, setError) => {
  setLoading(true)

  const emailToDelete = AdminEmails.find(f => f.id === id)
  if (!emailToDelete) {
  ...
  try {
    ...
    setAdminEmails(AdminEmails.filter(f => f.id !== id))
    setError(null)
    setDeletingId(null)
    setOpenDeleteDialog(false)
  } catch (error) {
