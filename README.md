...
This step should be done ideally by the person that will be the first admin in the platform. The API key generated by them will be used to start the app.
1. Login or create an account with SendGrid. It's preferrable to use a non-USD email because USD email accounts (@sandiego.edu) are blocked from working with SendGrid. Using a non-USD email makes it easier to reuse the same email as the "sender" email in the setup process.
   - https://app.sendgrid.com/
   - Save the Recovery Code provided for emergencies.
   - Fill out details in the registration process
2. When the "Send emails from your domain" screen shows up, click "Skip to dashboard" in the upper right corner.
3. Click on the "Create sender identity" button in the Welcome screen and fill out the information for the "sender":
   - From name: this will be the name of the email sender, e.g. OUR SEARCH Team
   - From email address: the email address of the sender. It cannot be a @sandiego.edu account since USD blocks sending emails using third parties. It's recommended that you use the same email that was used to create the SendGrid account
   - Reply to: the email address that will receive replies from users. For example, if a student gets an annual notification from the app and for some reason decides to reply to the email.
   - Nickname: can be anything, this is just for your reference in the SendGrid dashboard.
4. Once the sender is added you will receive a verification email at the sender's email address. You will need to open the email and click the "Verify Single Sender" button.
5. Once the sender is verified, you will need to create an API key:
   - Go back to the Dashboard: https://app.sendgrid.com/
   - On the left tab menu, click on "Settings" and then click on "API Keys"
   - Click the "Create API Key" button
   - In "API Key Name" you can type anything, but a descriptive name is recommended, e.g. "OUR SEARCH App SendGrid API Key"
   - For API Key Permissions select Restricted Accesss. You will get a list of features to enable. Enable "Full Access" for the following features:
      - Mail Send:
         - Mail Send
      - Mail Settings:
         - BCC
         - Bounce Purge
         - Event Notification
   - Click on the Create Key button and the API key should appear, it will be a long text with a bunch of letters, characters, and numbers.
   - **CAUTION: The API Key will ONLY BE SHOWN ONCE. Make sure to copy it and save it somewhere. If you accidentally click "Done" or close the window, you will have to create another key by repeating these steps. Keep the API Key saved somewhere that is NOT publicly accessible.**
6. Click "Done" to make sure that the API Key was created. Once these steps are completed, share the "Sender email" that you added to the dashboard in step 3, and the API Key you created in step 5 with the IT person that will be booting up the app.
...
*  For --sendgrid_email use the "sender email" created in step 3.
