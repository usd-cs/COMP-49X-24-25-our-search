...
public class GatewayControllerTest {
  ...
  void getProjects_returnsExpectedResult() throws Exception {
    ...
    mockMvc.perform(get("/projects"))
           ...
           .andExpect(jsonPath("$.departments[0].majors[0].id").value(1))
           .andExpect(jsonPath("$.departments[0].majors[0].name").value("Computer Science"))
           .andExpect(jsonPath("$.departments[0].majors[0].posts[0].id").value(1))
           .andExpect(jsonPath("$.departments[0].majors[0].posts[0].name").value("AI Project"))
           .andExpect(jsonPath("$.departments[0].majors[0].posts[0].description").value("Research in AI and Machine Learning"))
           .andExpect(jsonPath("$.departments[0].majors[0].posts[0].desired_qualifications").value("Knowledge of ML and basic AI algorithms"))
           .andExpect(jsonPath("$.departments[0].majors[0].posts[0].is_active").value(true))
           .andExpect(jsonPath("$.departments[0].majors[0].posts[0].majors[0]").value("Computer Science"))
           .andExpect(jsonPath("$.departments[0].majors[0].posts[0].umbrella_topics[0]").value("AI"))
           .andExpect(jsonPath("$.departments[0].majors[0].posts[0].research_periods[0]").value("Fall 2025"))
           .andExpect(jsonPath("$.departments[0].majors[0].posts[0].faculty.first_name").value("Dr."))
           .andExpect(jsonPath("$.departments[0].majors[0].posts[0].faculty.last_name").value("Faculty"))
           .andExpect(jsonPath("$.departments[0].majors[0].posts[0].faculty.email").value("faculty@test.com"));
  }
}
